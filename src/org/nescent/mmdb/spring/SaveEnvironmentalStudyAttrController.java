...
public class SaveEnvironmentalStudyAttrController implements Controller {
	public ModelAndView handleRequest(HttpServletRequest arg0,
		...
		try {
			for (Enumeration en = arg0.getParameterNames(); en
				...
				if (cvid.equals("id") || cvid.equals(newValueFieldId))
					continue;
				...
				if (intId > 0) {
					MmExperimentValue v = valueDao.findById(Integer
							.valueOf(cvid));
					if (v == null)
						throw new Exception(
								"No MmExperimentValue found with the id: " + id);
					...
					if (fname != null) {
						if (fname.equals("NewEnvironmentStudyAttribute")) {
							...
							throw new Exception(
									"Not supported field in the Envieonmental Study: "
											+ fname);
						}
					} else
